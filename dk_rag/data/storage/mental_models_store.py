    ...
    def index_documents(
        ...
        try:
            ...
            for i in range(0, len(documents), batch_size):
                ...
                try:
                    # Generate unique IDs based on mental model names
                    ...
                    for doc in batch:
                        ...
                        if not name:
                            ...
                            doc_id = f"mm_unnamed_{total_indexed + len(batch_ids)}"
                            self.logger.warning(f"Document at index {len(batch_ids)} has no name, using ID: {doc_id}")
                        else:
                            # Create deterministic ID from mental model name
                            # Format: mm_<sanitized_name>
                            sanitized_name = name.lower().replace(' ', '_').replace('/', '_').replace('\\', '_')
                            doc_id = f"mm_{sanitized_name}"
            ...
            if duplicate_names:
                ...
                duplicate_warning = (
                    f"Found {len(duplicate_names)} duplicate mental model names "
                ...
                self.logger.info("No duplicate mental model names detected ✓")
