...
def main():
    ...
    try:
        ...
        print(f"🧠 Query Analysis Model: {settings.agent.tools.query_analysis.llm_model} (fast)")
        print(f"🧠 Synthesis Model: {settings.agent.synthesis.llm_model} (heavy)")
        print(f"🎯 Retrieval Config: MM={settings.agent.tools.mental_models.k}, CB={settings.agent.tools.core_beliefs.k}, T={settings.agent.tools.transcripts.k}")
